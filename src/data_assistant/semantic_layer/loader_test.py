...
def test_semantic_layer_loads_dataset_table_relationship() -> None:
    ...
    assert orders_metrics_by_id["order_count"].aliases == (
        "transactions",
        "purchases",
    )
    assert orders_metrics_by_id["total_discount_amount"].aliases == (
        "promo spend",
        "markdown spend",
    )
    ...
    assert support_metrics_by_id["support_ticket_count"].aliases == (
        "cases",
        "incidents",
        "help requests",
    )
