"""Tests for the pure Slack Assistant adapter.
...
import data_assistant.slack.adapter as adapter
...
def _actions_blocks(
    ...
    return [block for block in block_list if block.get("type") == "actions"]
...
def test_on_thread_started_posts_greeting_and_suggested_prompts(
    ...
    test_adapter = adapter.AssistantAdapter(
    ...
    test_adapter.on_thread_started(
        say=say,
    ...
    assert say.calls == [(prompts.GREETING, None)]
    assert set_suggested_prompts.calls == [
        [dict(prompt) for prompt in prompts.SUGGESTED_PROMPTS]
    ]
    # The 3 provisional summarize-intent prompts are delivered dynamically.
    assert len(prompts.SUGGESTED_PROMPTS) == 3
...
def test_on_user_message_sets_status_runs_pipeline_then_says(
    ...
    block_list: tuple[contracts.SlackBlock, ...] = (
    ...
    def answer_path(
        ...
        return _final_response(blocks=block_list)
    ...
    assert said_blocks[1 : 1 + len(block_list)] == block_list
...
def test_on_user_message_says_non_answer_without_fallback(
    ...
    assert adapter.RUNTIME_FALLBACK_MESSAGE not in [c[0] for c in say.calls]
...
def test_on_user_message_says_runtime_fallback_on_crash_without_raising(
    ...
    assert fallback_text == adapter.RUNTIME_FALLBACK_MESSAGE
...
def test_on_user_message_logs_failure_metadata_for_maintainers(
    ...
    with caplog.at_level(logging.ERROR, logger=adapter.logger.name):
        test_adapter.on_user_message(
            text="What was total revenue last quarter?",
            user="U123",
            channel="D123",
    ...
    records = [
        record for record in caplog.records if record.name == adapter.logger.name
    ]
...
def test_default_identity_maps_slack_user() -> None:
    identity = adapter.default_identity("U777")
...
def test_dev_identity_uses_local_allowed_identity() -> None:
    ...
    identity = adapter.dev_identity("U777")
...
def _run_adapter(
    *,
    ...
    answer_path: adapter.AnswerPath,
...
def test_on_user_message_logs_error_record_and_still_says_fallback(
    ...
    assert len(say.calls) == 1
    assert say.calls[0][0] == adapter.RUNTIME_FALLBACK_MESSAGE
    records = _read_log_records(log_path)
    assert len(records) == 1
    record = records[0]
...
def test_record_runtime_error_logs_qa_review_metadata(
    ...
    test_adapter.record_runtime_error(
...
def test_answer_and_render_returns_id_response_blocks_and_logs_one_record(
    ...
    def answer_path(
        ...
        progress_sink(EXPECTED_PROGRESS_STATUSES[0])
        seen_questions.append(question)
        return _final_response(text="Final answer text.", blocks=block_list)
    ...
    interaction_id, final_response, reply_blocks = test_adapter.answer_and_render(
    ...
    assert reply_blocks[1 : 1 + len(block_list)] == block_list
...
def test_answer_and_render_logs_optional_qa_case_id_without_changing_question(
    ...
    interaction_id, _final_response_out, _reply_blocks = test_adapter.answer_and_render(
        text="What was total revenue by region in January 2026?",
        user="qa_driver",
...
def test_answer_and_render_logs_qa_review_metadata(
    ...
    test_adapter.answer_and_render(
        text="What was total revenue by region in January 2026?",
        user="qa_driver",
        set_status=RecordingStatus(),
        qa_review_context=interaction_record.QAReviewContext(
...
def test_answer_and_render_keeps_normal_reply_without_qa_header_or_qa_buttons(
    ...
    interaction_id, _final_response_out, reply_blocks = test_adapter.answer_and_render(
...
def test_answer_and_render_qa_header_omits_case_id_for_unidentified_case(
    ...
    _interaction_id, _final_response_out, reply_blocks = test_adapter.answer_and_render(
        text="Legacy question?",
        user="qa_driver",
        set_status=RecordingStatus(),
        qa_review_context=interaction_record.QAReviewContext(
...
def test_answer_and_render_truncates_long_question_in_echo_block(
    ...
    """
    long_question = "x" * (blocks.QUESTION_ECHO_MAX_CHARS + 50)
    ...
    body = echo_text[len("❓ ") :]
    assert len(body) == blocks.QUESTION_ECHO_MAX_CHARS + 1  # cap + "…"
